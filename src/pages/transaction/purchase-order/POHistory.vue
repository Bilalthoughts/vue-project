<template>
  <div class="w-100 bg-light p-5" style="height: 100vh">
    <div class="container d-flex justify-content-between mb-2">
      <div>
        <span class="d-block"><b>Purchase Order History</b> </span>
      </div>
      <div></div>
    </div>
    <div class="container shadow-lg rounded bg-white">
      ...
      <div class="d-flex justify-content-between align-items-center py-3">
        ...
        <small>
          <paginationWrapper
            v-model="page"
            :records="po_history_list.total_count"
            :per-page="noOfRows"
            @paginate="myCallback()"
            :options="{
              chunk: 3,
...
<script>
import { po_history_list } from '../../../data/po-history.js';
...
export default {
  ...
  computed: {
    allUniquePOStatus() {
      return [...new Set(this.po_history_list.data.map((item) => item.PO_Status))];
    },
    allUniqueTP() {
      return [...new Set(this.po_history_list.data.map((item) => item.Trading_Partner))];
    },
    filteredArray() {
      let arrFiltered = this.po_history_list.data;
      if (this.search !== '') {
